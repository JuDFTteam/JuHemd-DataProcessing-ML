...
explainer=shap.TreeExplainer(ETR,X2)
plt.clf()
shap_values2=explainer(X2)
shap.plots.bar(shap_values2,show=False)
plt.xlabel('Mean SHAP Value')
plt.tight_layout()
plt.savefig('\n MeanRedDescrSHAP.png',dpi=1200)
plt.clf()
...
shap.summary_plot(shap_values2,show=False,max_display=9)
plt.xlabel('SHAP Value')
plt.tight_layout()
...
for k in ['Ferro Density','Cobalt Density', 'Nickel Density']:
    ...
    ystd=np.std(tc)
    ymean=np.mean(tc)
    hk.ax_marg_x.axvline(xmean, color='red', ls='--')
    hk.ax_marg_x.axvspan(xmean - xstd, xmean + xstd, color='red', alpha=0.1)
    hk.ax_marg_y.axhline(ymean, color='red', ls='--')
    hk.ax_marg_y.axhspan(ymean - ystd, ymean + ystd, color='red', alpha=0.1)
    plt.tight_layout()
    plt.xlim(-0.25,1)
