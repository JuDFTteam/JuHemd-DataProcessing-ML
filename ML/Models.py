...
shap.plots.force(explainer.expected_value, shap_values[index,:], testData[index,:],link='logit',feature_names=descr,show=False)
plt.tight_layout()
plt.savefig('ForceSHAPHIGH.png',dpi=1200)
plt.clf()
index=np.where(testTc==np.min(testTc))
shap.force_plot(explainer.expected_value, shap_values[index,:], testData[index,:], link="logit",feature_names=descr,show=False)
plt.tight_layout()
